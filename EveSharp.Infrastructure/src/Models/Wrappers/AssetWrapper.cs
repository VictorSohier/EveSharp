...
namespace EveSharp.Infrastructure.Models.Wrappers
{
	public class AssetWrapper
	{
		...
		public async Task<AssetLocation[]> GetCharacterAssetLocationsAsync(OAuth2Token token, int characterId, DataSources datasource = DataSources.tranquility, params long[] itemIds)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			...
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
			{
				ret = _serializer.Deserialize<AssetLocation[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
				return ret;
			}
			throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		}
		...
		public async Task<AssetLocation[]> GetCorporationAssetLocationsAsync(OAuth2Token token, int corporationId, DataSources datasource = DataSources.tranquility, params long[] itemIds)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			HttpResponseMessage message = await _client.PostAsync(
