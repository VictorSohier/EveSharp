...
namespace EveSharp.Infrastructure.Models.Wrappers
{
	public class FleetWrapper
	{
		...
		public async Task<Fleet> GetCurrentFleetAsync(OAuth2Token token, int characterId, DataSources datasource = DataSources.tranquility)
		{
			...
			HttpResponseMessage message = await _client.GetAsync($"/{WrapperConfig._instance.API_VERSION}/characters/{characterId}/fleet?datasource={Enum.GetName(datasource)?.ToLower()}");
			Fleet ret;
		...
		public async Task<FleetDetails> GetFleetAsync(OAuth2Token token, long fleetId, DataSources datasource = DataSources.tranquility)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			HttpResponseMessage message = await _client.GetAsync($"/{WrapperConfig._instance.API_VERSION}/fleets/{fleetId}?datasource={Enum.GetName(datasource)?.ToLower()}");
			FleetDetails ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
			{
		...
		public async Task<FleetMember[]> GetFleetMembersAsync(OAuth2Token token, long fleetId, DataSources datasource = DataSources.tranquility)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			HttpResponseMessage message = await _client.GetAsync($"/{WrapperConfig._instance.API_VERSION}/fleets/{fleetId}/members?datasource={Enum.GetName(datasource)?.ToLower()}");
			FleetMember[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
			{
		...
		public async Task RemoveFleetMemberAsync(OAuth2Token token, long fleetId, int characterId, DataSources datasource = DataSources.tranquility)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			await _client.DeleteAsync($"/{WrapperConfig._instance.API_VERSION}//fleets/{fleetId}/members/{characterId}?datasource={Enum.GetName(datasource)?.ToLower()}");
		}
		...
		public async Task RemoveSquadAsync(OAuth2Token token, long fleetId, long squadId, DataSources datasource = DataSources.tranquility)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			await _client.DeleteAsync($"/{WrapperConfig._instance.API_VERSION}//fleets/{fleetId}/squads/{squadId}?datasource={Enum.GetName(datasource)?.ToLower()}");
		}
		...
		public async Task<Wing[]> GetWingsAsync(OAuth2Token token, long fleetId, DataSources datasource = DataSources.tranquility)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			HttpResponseMessage message = await _client.GetAsync($"/{WrapperConfig._instance.API_VERSION}/fleets/{fleetId}/wings?datasource={Enum.GetName(datasource)?.ToLower()}");
			Wing[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
			{
		...
		public async Task RemoveWingAsync(OAuth2Token token, long fleetId, long wingId, DataSources datasource = DataSources.tranquility)
		{
			_client.DefaultRequestHeaders.Remove("Authorization");
			_client.DefaultRequestHeaders.Add("Authorization", $"{token.tokenType} {token.accessToken}");
			await _client.DeleteAsync($"/{WrapperConfig._instance.API_VERSION}//fleets/{fleetId}/wings/{wingId}?datasource={Enum.GetName(datasource)?.ToLower()}");
		}
