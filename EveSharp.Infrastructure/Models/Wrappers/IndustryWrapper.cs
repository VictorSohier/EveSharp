...
namespace EveSharp.Infrastructure.Models.Wrappers
{
	public class IndustryWrapper
	{
		...
		public async Task<Job[]> GetCharacterJobsAsync(int characterId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			{
				HttpResponseMessage message = await _client.GetAsync($"characters/{characterId}/industry/jobs?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
		...
		public async Task<MiningEntry[]> GetMiningHistoryAsync(int characterId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"characters/{characterId}/mining?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		public async Task<MoonExtraction[]> GetMoonExtractionsAsync(int corporationId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"corporation/{corporationId}/mining/extractions?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		public async Task<MiningObserver[]> GetMiningObserversAsync(int corporationId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"corporation/{corporationId}/mining/observers?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		public async Task<MiningObserverDetails> GetMiningObserverDetailsAsync(int corporationId, long observerId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"corporation/{corporationId}/mining/observers/{observerId}?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		public async Task<Job[]> GetCorporationJobsAsync(int corporoationId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"corporation/{corporoationId}/industry/jobs?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
				ret = _serializer.Deserialize<Job[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
			}
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"industry/facilities?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
			Facility[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"industry/systems?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
			SystemCostIndex[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
