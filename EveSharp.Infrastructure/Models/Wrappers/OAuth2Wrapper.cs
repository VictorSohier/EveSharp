...
namespace EveSharp.Infrastructure.Models.Wrappers
{
	public class OAuth2Wrapper
	{
		...
		public async Task<(JsonWebKeySet, bool)> VerifyToken(OAuth2Token token)
		{
			...
			string headerString = Encoding.UTF8.GetString(Convert.FromBase64String(tokenComponents[0] + new string('=', 3 - (tokenComponents[0].Length % 3))));
			string content = Encoding.UTF8.GetString(Convert.FromBase64String(tokenComponents[1] + new string('=', 3 - (tokenComponents[1].Length % 3))));
