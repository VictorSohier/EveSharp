...
namespace EveSharp.Infrastructure.Models.Wrappers
{
	public class ContactWrapper
	{
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"alliances/{allianceId}/contacts?datasource={Enum.GetName(datasource)?.ToLower()}&page={page}");
			Contact[] ret;
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"alliances/{allianceId}/contacts/labels?datasource={Enum.GetName(datasource)?.ToLower()}");
			ContactLabel[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		public async Task GetCharacterContactsAsync(int characterId, DataSources datasource = DataSources.tranquility, params int[] contactIds)
		{
			...
			await _client.DeleteAsync($"characters/{characterId}/contacts?datasource={Enum.GetName(datasource)?.ToLower()}&contact_ids={payload}");
		}
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"characters/{characterId}/contacts?datasource={Enum.GetName(datasource)?.ToLower()}&page={page}");
			Contact[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
			ret = _serializer.Deserialize<Contact[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"characters/{characterId}/contacts/labels?datasource={Enum.GetName(datasource)?.ToLower()}");
			ContactLabel[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
			ret = _serializer.Deserialize<ContactLabel[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"corporations/{corporationId}/contacts?datasource={Enum.GetName(datasource)?.ToLower()}&page={page}");
			Contact[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
			ret = _serializer.Deserialize<Contact[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"corporations/{corporationId}/contacts/labels?datasource={Enum.GetName(datasource)?.ToLower()}");
			ContactLabel[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
			ret = _serializer.Deserialize<ContactLabel[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
