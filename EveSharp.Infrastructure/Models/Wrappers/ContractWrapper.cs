...
namespace EveSharp.Infrastructure.Models.Wrappers
{
	public class ContractWrapper
	{
		...
		public async Task<Contract[]> GetCharacterContractsAsync(int characterId, int page = 1, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			{
				HttpResponseMessage message = await _client.GetAsync($"characters/{characterId}/contracts?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}&page={page}");
		...
		public async Task<Bid[]> GetCharacterContractBidsAsync(int characterId, int contractId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"characters/{characterId}/contracts/{contractId}/bids?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		public async Task<Item[]> GetCharacterContractItemsAsync(int characterId, int contractId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"characters/{characterId}/contracts/{contractId}/items?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		{
			Contract[] ret;
			HttpResponseMessage message = await _client.GetAsync($"contracts/public/{regionId}?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}&page={page}");
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
			ret = _serializer.Deserialize<Contract[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
			return ret;
		...
		{
			Bid[] ret;
			HttpResponseMessage message = await _client.GetAsync($"/contracts/public/bids/{contractId}?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
			ret = _serializer.Deserialize<Bid[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
			return ret;
		...
		{
			Item[] ret;
			HttpResponseMessage message = await _client.GetAsync($"contracts/public/items/{contractId}?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
			ret = _serializer.Deserialize<Item[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
			return ret;
		...
		public async Task<Contract[]> GetCorporationContractsAsync(int corporationId, int page = 1, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"corporations/{corporationId}/contracts?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}&page={page}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
				ret = _serializer.Deserialize<Contract[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
			}
		...
		public async Task<Bid[]> GetCorporationContractBidsAsync(int corporationId, int contractId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"corporations/{corporationId}/contracts/{contractId}/bids?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
				ret = _serializer.Deserialize<Bid[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
			}
		...
		public async Task<Item[]> GetCorporationContractItemsAsync(int corporationId, int contractId, DataSources datasource = DataSources.TRANQUILITY)
		{
			...
			if (_client.DefaultRequestHeaders.Any(e => e.Key == "authorization" & e.Value.Any(f => !string.IsNullOrWhiteSpace(f))))
			{
				HttpResponseMessage message = await _client.GetAsync($"corporations/{corporationId}/contracts/{contractId}/items?datasource={Enum.GetName<DataSources>(datasource)?.ToLower()}");
				if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
					throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
				ret = _serializer.Deserialize<Item[]>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync())));
			}
