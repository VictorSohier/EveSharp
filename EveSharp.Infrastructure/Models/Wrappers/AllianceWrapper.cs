...
namespace EveSharp.Infrastructure.Models.Wrappers
{
	public class AllianceWrapper
	{
		...
		{
			_client = new();
			_client.BaseAddress = new($"{WrapperConfig._instance.DOMAIN}/{WrapperConfig._instance.API_VERSION}/alliances");
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"/{allianceId}?datasource={Enum.GetName(datasource)?.ToLower()}");
			Alliance ret;
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"/{allianceId}/corporations?datasource={Enum.GetName(datasource)?.ToLower()}");
			int[] ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
		...
		{
			HttpResponseMessage message = await _client.GetAsync($"/{allianceId}/icons?datasource={Enum.GetName(datasource)?.ToLower()}");
			Icon ret;
			if (WrapperConfig._instance.SUCCESS.Contains(message.StatusCode))
				throw new Exception(_serializer.Deserialize<Error>(new JsonTextReader(new StreamReader(await message.Content.ReadAsStreamAsync()))).error);
